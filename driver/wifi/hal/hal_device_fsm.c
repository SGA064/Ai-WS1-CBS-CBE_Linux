...
*****************************************************************************/
osal_void hal_scan_handle_scan_back(hal_to_dmac_device_stru *hal_device, osal_u16 event, osal_void *event_data)
{
    ...
    if (hal_scan_params->need_switch_back_home_channel == OSAL_TRUE) {
        ...
        if (event == HAL_DEVICE_EVENT_SCAN_END) {
            ...
            if (hal_vap != OSAL_NULL) {
                hal_dev_req_sync_pm_bitmap(hal_vap->vap_id, HAL_DEVICE_WORK_SUB_STATE_ACTIVE);
            }
        }
...
osal_u32 hal_device_state_scan_event(osal_void *ctx, osal_u16 event, osal_u16 event_data_len, osal_void *event_data)
{
    ...
    switch (event) {
        ...
        case HAL_DEVICE_EVENT_SCAN_END:
            hal_scan_handle_scan_back(hal_device, event, event_data);
            break;
