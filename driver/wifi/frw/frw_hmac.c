...
OAL_STATIC osal_void frw_msg_vap_check_fail_handle(osal_u16 msg_id, frw_msg *msg)
{
    oal_netbuf_stru *netbuf = OSAL_NULL;

    if (msg_id != WLAN_MSG_H2H_RX && msg_id != WLAN_MSG_H2H_CRX_RX) {
        return;
    }
...
osal_s32 frw_msg_exec_callback(osal_u16 msg_id, osal_u8 vap_id, frw_msg *msg)
{
    ...
    if (hmac_vap == OSAL_NULL) {
        frw_msg_vap_check_fail_handle(msg_id, msg);
