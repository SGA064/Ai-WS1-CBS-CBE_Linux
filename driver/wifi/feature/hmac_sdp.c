    ...
    osal_u8 len)
{
    osal_s32 ret;
    frw_msg msg_info = {0};
    osal_u8 *sdp_data = OSAL_NULL;

    /* 此处申请内存注意释放，由wal_sdp_proc_rx_data释放 */
    sdp_data = (osal_u8 *)oal_mem_alloc(OAL_MEM_POOL_ID_LOCAL, len + 1, OAL_TRUE);
    if (sdp_data == OSAL_NULL) {
        oam_error_log0(0, OAM_SF_SDP, "{hmac_sdp_send_followup_to_host:alloc mem failed!}");
        return OAL_FAIL;
    }
    (osal_void)memset_s(sdp_data, len + 1, 0, len + 1);

    if (memcpy_s(sdp_data, len, data, len) != EOK) {
        oam_error_log0(0, OAM_SF_SDP, "hmac_sdp_send_followup_to_host:action_hdr memcpy fail.");
        oal_mem_free(sdp_data, OAL_TRUE);
        return OAL_FAIL;
    }
    ...
    msg_info.data = (osal_void *)sdp_data;
    msg_info.data_len = len + 1;

    ret = frw_asyn_host_post_msg(WLAN_MSG_H2W_SDP_RX_DATA, FRW_POST_PRI_LOW, hmac_vap->vap_id, &msg_info);
    oal_mem_free(sdp_data, OAL_TRUE);
    if (ret != OAL_SUCC) {
