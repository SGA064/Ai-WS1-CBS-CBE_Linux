...
OSAL_STATIC osal_bool hmac_single_proxysta_vap_add(osal_void *notify_data)
{
    ...
    // 初始化net_device属性，允许STA设备加入内核网桥
    hmac_vap->net_device->priv_flags &= ~IFF_DONT_BRIDGE;
#else
...
OSAL_STATIC osal_bool hmac_single_proxysta_vap_del(osal_void *notify_data)
{
#if defined(_PRE_OS_VERSION_LINUX) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    hmac_vap_stru *hmac_vap = (hmac_vap_stru *)notify_data;
...
osal_u32 hmac_single_proxysta_init(osal_void)
{
    ...
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_ADD_VAP, hmac_single_proxysta_vap_add);
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_DEL_VAP, hmac_single_proxysta_vap_del);
...
osal_void hmac_single_proxysta_deinit(osal_void)
{
    ...
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_ADD_VAP, hmac_single_proxysta_vap_add);
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_DEL_VAP, hmac_single_proxysta_vap_del);
