    ...
    const hmac_roam_info_stru *roam_info, hmac_roam_rsp_stru *roam_rsp, const hmac_asoc_rsp_stru *asoc_rsp)
{
    ...
    if (osal_memcmp(roam_rsp->bssid, roam_info->old_bss.bssid, WLAN_MAC_ADDR_LEN) == 0) {
        ...
        msg_info.data_len = OAL_SIZEOF(hmac_roam_rsp_stru) + roam_rsp->asoc_rsp_ie_len;
...
OAL_STATIC osal_u32 hmac_roam_connect_notify_wpas(hmac_roam_info_stru *roam_info, osal_u8 *mac_hdr, osal_u16 msg_len)
{
    ...
    hmac_roam_rsp_stru               *roam_rsp = OSAL_NULL;
    hmac_vap_stru                    *hmac_vap = roam_info->hmac_vap;
    hmac_user_stru                   *hmac_user = OSAL_NULL;
    osal_u8                          *mgmt_data = OSAL_NULL;
    osal_u32                          asoc_rsp_len;

    /* 记录关联响应帧的部分内容，用于上报给内核 */
    if (msg_len < OAL_ASSOC_RSP_IE_OFFSET) {
        oam_error_log2(0, 0, "vap_id[%d]{hmac_roam_connect_notify_wpas:msg_len is err,%d}", hmac_vap->vap_id, msg_len);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    asoc_rsp_len = msg_len - OAL_ASSOC_RSP_IE_OFFSET;

    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user_etc(hmac_vap->assoc_vap_id);
    if (hmac_user == OAL_PTR_NULL) {
        oam_warning_log1(0, OAM_SF_ROAM, "{hmac_roam_connect_notify_wpas hmac_user[%d] NULL}", hmac_vap->assoc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }

    asoc_rsp = (hmac_asoc_rsp_stru *)oal_memalloc(asoc_rsp_len + sizeof(hmac_asoc_rsp_stru));
    if (asoc_rsp == OAL_PTR_NULL) {
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    memset_s(asoc_rsp, OAL_SIZEOF(hmac_asoc_rsp_stru) + asoc_rsp_len, 0, OAL_SIZEOF(hmac_asoc_rsp_stru) + asoc_rsp_len);

    roam_rsp = (hmac_roam_rsp_stru *)oal_memalloc(asoc_rsp_len + sizeof(hmac_roam_rsp_stru));
    if (roam_rsp == OAL_PTR_NULL) {
        oal_free(asoc_rsp);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    memset_s(roam_rsp, OAL_SIZEOF(hmac_roam_rsp_stru) + asoc_rsp_len, 0, OAL_SIZEOF(hmac_roam_rsp_stru) + asoc_rsp_len);
    ...
    mgmt_data = (osal_u8 *)roam_rsp + sizeof(hmac_roam_rsp_stru);
    (osal_void)memcpy_s(mgmt_data, asoc_rsp_len, (osal_u8 *)(mac_hdr + OAL_ASSOC_RSP_IE_OFFSET), asoc_rsp_len);
    roam_rsp->asoc_rsp_ie_buff = mgmt_data;
    roam_rsp->asoc_rsp_ie_len = asoc_rsp_len;

    mgmt_data = (osal_u8 *)asoc_rsp + sizeof(hmac_asoc_rsp_stru);
    (osal_void)memcpy_s(mgmt_data, asoc_rsp_len, (osal_u8 *)(mac_hdr + OAL_ASSOC_RSP_IE_OFFSET), asoc_rsp_len);
    asoc_rsp->asoc_rsp_ie_buff = mgmt_data;
    asoc_rsp->asoc_rsp_ie_len = asoc_rsp_len;

#ifdef _PRE_WLAN_FEATURE_WS92_MERGE
    roam_rsp->st_channel.chan_number = roam_info->connect.bss_dscr->st_channel.chan_number;
#endif

    hmac_roam_connect_msg_to_wpas(hmac_vap, roam_info, roam_rsp, asoc_rsp);
    oal_free(roam_rsp);
    oal_free(asoc_rsp);
    return OAL_SUCC;
}
