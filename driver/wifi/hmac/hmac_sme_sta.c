...
osal_s32 hmac_cfg80211_start_scan_sta_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (ret != OAL_SUCC) {
        oam_notice_log1(0, OAM_SF_SCAN, "{hmac_cfg80211_start_scan_sta_etc::hmac_cfg80211_scan_app_ie [%d].}", ret);
        return OAL_FAIL;
    }
    ...
    if (ret != OAL_SUCC) {
        oam_notice_log2(0, OAM_SF_SCAN, "vap[%d]hmac_cfg80211_start_scan_sta_etc::fsm err[%d]", hmac_vap->vap_id, ret);
        return (osal_s32)ret;
    }
...
osal_void hmac_report_connect_failed_result_etc(hmac_vap_stru *hmac_vap, mac_status_code_enum_uint16 reason_code)
{
    ...
    asoc_rsp = (hmac_asoc_rsp_stru *)oal_memalloc(sizeof(hmac_asoc_rsp_stru));
    if (asoc_rsp == OAL_PTR_NULL) {
        oam_error_log1(0, OAM_SF_ASSOC, "vap_id[%d] {hmac_report_connect_failed_result_etc::asoc_rsp alloc null.}",
            hmac_vap->vap_id);
        return;
    }
    ...
    (osal_void)frw_asyn_host_post_msg(WLAN_MSG_H2W_ASOC_COMP_STA, FRW_POST_PRI_LOW, hmac_vap->vap_id, &msg_info);
    oal_free(asoc_rsp);
    return;
}
...
osal_void hmac_handle_auth_rsp_sta_etc(hmac_vap_stru *hmac_vap, osal_u8 *msg)
{
    ...
    oam_notice_log3(0, OAM_SF_AUTH, "vap_id[%d] hmac_handle_auth_rsp_sta_etc::auth fail[%d],cnt[%d]",
...
void hmac_handle_assoc_rsp_succ_sta(const hmac_vap_stru *hmac_vap, hmac_asoc_rsp_stru *asoc_rsp)
{
    osal_u8 *mgmt_data;
    frw_msg msg_info = {0};
    hmac_asoc_rsp_stru *asoc_rsp_report = OSAL_NULL;
    ...
    asoc_rsp_report = (hmac_asoc_rsp_stru *)oal_memalloc(asoc_rsp->asoc_rsp_ie_len + sizeof(hmac_asoc_rsp_stru));
    if (asoc_rsp_report == OAL_PTR_NULL) {
        oam_error_log1(0, OAM_SF_ASSOC, "vap_id[%d] {hmac_handle_assoc_rsp_succ_sta::asoc_rsp_report alloc null.}",
            hmac_vap->vap_id);
        return;
    }
    (osal_void)memset_s(asoc_rsp_report, asoc_rsp->asoc_rsp_ie_len + sizeof(hmac_asoc_rsp_stru), 0,
        asoc_rsp->asoc_rsp_ie_len + sizeof(hmac_asoc_rsp_stru));

    if (memcpy_s((osal_u8 *)asoc_rsp_report, sizeof(hmac_asoc_rsp_stru), (osal_u8 *)asoc_rsp,
        sizeof(hmac_asoc_rsp_stru)) != EOK) {
        oam_warning_log0(0, OAM_SF_CFG, "{hmac_handle_assoc_rsp_succ_sta stru::memcpy err.}");
    }

    mgmt_data = (osal_u8 *)asoc_rsp_report + sizeof(hmac_asoc_rsp_stru);
    ...
    asoc_rsp_report->asoc_rsp_ie_buff = mgmt_data;

    msg_info.data = (osal_u8 *)asoc_rsp_report;
    msg_info.data_len = OAL_SIZEOF(hmac_asoc_rsp_stru) + asoc_rsp->asoc_rsp_ie_len;

    (osal_void)frw_asyn_host_post_msg(WLAN_MSG_H2W_ASOC_COMP_STA, FRW_POST_PRI_LOW, hmac_vap->vap_id, &msg_info);
    oal_free(asoc_rsp_report);
    return;
}
...
osal_void hmac_handle_asoc_rsp_sta_etc(hmac_vap_stru *hmac_vap, osal_u8 *msg)
{
    ...
    if (asoc_rsp->result_code == HMAC_MGMT_SUCCESS) {
        ...
        oam_notice_log3(0, OAM_SF_AUTH, "vap_id[%d] {hmac_handle_asoc_rsp_sta_etc::asoc fail[%d], assoc_cnt[%d]}",
            hmac_vap->vap_id,
