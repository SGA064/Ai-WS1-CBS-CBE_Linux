...
osal_u32  hmac_user_del_etc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    oam_notice_log2(0, OAM_SF_UM, "{hmac_user_del_etc:del user[%d],is multi user[%d]}",
        hmac_user->assoc_id, hmac_user->is_multi_user);
    oam_notice_log4(0, OAM_SF_UM, "{user mac:%02X:%02X:%02X:%02X:XX:XX}",
        /* 0 1 2 3:MAC索引值 */
