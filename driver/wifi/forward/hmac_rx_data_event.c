    ...
    osal_u8 data_type)
{
    ...
    if ((data_type == MAC_DATA_EAPOL) && (mac_get_eapol_type_etc(curr_netbuf) == OAL_EAPOL_TYPE_KEY)) {
        oam_notice_log2(0, OAM_SF_CONN, "vap_id[%d] {hmac_rx_process_frame_eapol_keyinfo::rx eapol, info is %x }",
...
WIFI_HMAC_TCM_TEXT osal_s32 hmac_rx_process_data_msg(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    /* host侧RX方向用户速率信息统计 */
    frw_util_notifier_notify(WLAN_UTIL_NOTIFIER_EVENT_USER_RATE_RX_INFO, cb_ctrl);
