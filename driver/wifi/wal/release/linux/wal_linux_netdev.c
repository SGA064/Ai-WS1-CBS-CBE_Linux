...
#if defined(LINUX_VERSION_CODE) && defined(KERNEL_VERSION) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
STATIC osal_s32 wal_net_device_private_ioctl(oal_net_device_stru *net_dev, oal_ifreq_stru *ifr,
    void __user *data, osal_s32 cmd)
{
    unref_param(data);
    return wal_net_device_ioctl(net_dev, ifr, cmd);
}
#endif
...
#endif
#if defined(LINUX_VERSION_CODE) && defined(KERNEL_VERSION) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
    .ndo_siocdevprivate     = wal_net_device_private_ioctl,
#endif
