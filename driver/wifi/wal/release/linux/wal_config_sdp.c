...
osal_s32 wal_sdp_process_rx_data(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    sdp_len = msg->data_len;
    sdp_data = (osal_u8 *)msg->data;
    if (sdp_data == OAL_PTR_NULL) {
        oam_warning_log1(0, OAM_SF_SDP, "vap[%d]wal_sdp_process_rx_data:sdp_data null", hmac_vap->vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }

    oam_info_log3(0, OAM_SF_SDP, "vap[%d] wal_sdp_process_rx_data come in, len %d, data[0][%d]",
        hmac_vap->vap_id, sdp_len, sdp_data[0]);
#if defined (_PRE_PRODUCT_ID_HOST) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
    wal_cfgvendor_nan_event_report(wdev->wiphy, wdev, sdp_len, sdp_data);
#endif
