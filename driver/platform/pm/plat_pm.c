...
static oal_int32 low_power_probe(void)
{
    ...
    pm_data = osal_kzalloc(sizeof(struct pm_drv_data), OSAL_GFP_KERNEL);
    if (pm_data == NULL) {
...
oal_int32 pm_crg_init(oal_void)
{
    ...
    g_pm_crg_info.pm_crg_enable = OAL_FALSE;
    g_pm_crg_info.pm_crg_max_req_freq = PM_CRG_FREQ_LVL_240M;
    g_pm_crg_info.pm_crg_cur_freq = PM_CRG_FREQ_LVL_240M;
    g_pm_crg_info.rsv1 = 0;

    g_pm_crg_info.pm_crg_req_freq[PM_FSM_SVC_WLAN] = PM_CRG_FREQ_LVL_240M;
