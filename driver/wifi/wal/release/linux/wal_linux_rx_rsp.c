...
osal_s32  wal_asoc_comp_proc_sta_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (assoc_rsp->asoc_rsp_ie_len != 0) {
        // asoc_rsp_ie_buff指向hmac申请的内存，需要重新指向frw msg申请的内存空间
        assoc_rsp->asoc_rsp_ie_buff = (osal_u8 *)assoc_rsp + sizeof(hmac_asoc_rsp_stru);
    }
    ...
    oam_notice_log2(0, OAM_SF_ASSOC, "vap_id[%d] {wal_asoc_comp_proc_sta_etc status_code[%d] OK!}",
...
osal_s32  wal_roam_comp_proc_sta_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    // asoc_rsp_ie_buff指向hmac申请的内存，需要重新指向frw msg申请的内存空间
    roam_rsp->asoc_rsp_ie_buff = (osal_u8 *)roam_rsp + sizeof(hmac_roam_rsp_stru);
...
#endif
    /* sta模式下，不需要传mac地址 */
    oam_notice_log2(0, OAM_SF_ASSOC, "vap_id[%d] {wal_disasoc_comp_proc_sta_etc reason_code[%d] OK!}",
