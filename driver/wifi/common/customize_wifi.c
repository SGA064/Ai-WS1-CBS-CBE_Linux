...
wlan_customize_private_stru g_al_priv_cust_params[WLAN_CFG_PRIV_BUTT] = {
    ...
    {1, 1},                 /* alg_cross_prot_enable */
    {2, 1},                 /* cpu freq level */
    {1, 1},                 /* temp_protect_level */
};                            /* 私有定制化参数数组 */
...
OAL_STATIC wlan_cfg_cmd g_wifi_config_priv[] = {
    ...
    {"alg_cross_prot_enable",       WLAN_CFG_PRIV_CROSS_PROT_ENABLE},
    {"cpu_freq_level",              WLAN_CFG_PRIV_CPU_FREQ_LEVEL},
    {"temp_protect_level",          WLAN_CFG_PRIV_TEMP_PROTECT_LEVEL},
    {OAL_PTR_NULL, 0}
};
...
osal_u8 g_cpu_freq_level = 2; /* 2 cpu max freq level */
...
osal_s32 hwifi_hcc_custom_ini_data_buf(osal_u16 syn_id)
{
    ...
    tx_buf = osal_kzalloc(WLAN_LARGE_NETBUF_SIZE, OSAL_GFP_KERNEL);
    if (tx_buf == NULL) {
...
#ifdef _PRE_WLAN_NO_SUPPORT_INI
    // 根据国家码配置大区&大区功率
    hwifi_config_init_rf_fe_tx_power();
#else
    /* wifi上电时重读定制化配置 */
...
#ifdef _PRE_WLAN_NO_SUPPORT_INI
osal_void hwifi_config_init_rf_fe_tx_power(osal_void)
{
    // 根据默认的国家码，设置默认大区
    osal_u8 region_idx = hwifi_get_region(hwifi_get_country_code_etc());
    if (region_idx == INI_FAILED) {
        region_idx = REGDOMAIN_COMMON;
    }
    hwifi_set_nvram_tag_by_region_index(region_idx);
    // 设置大区功率
    memcpy_s(&g_cust_rf_fe_params.rf_power, sizeof(wlan_cust_rf_fe_power_params),
        &g_default_cust_tx_power[region_idx], sizeof(wlan_cust_rf_fe_power_params));
}

osal_void hwifi_config_refresh_rf_fe_tx_power(osal_void)
{
    osal_u8 region_idx = REGDOMAIN_COMMON;
    osal_u8 index;

    // 根据大区功率tag查找大区index
    for (index = 0; index < osal_array_size(g_region_table); index++) {
        if (g_nvram_tag == g_region_table[index].region_tag) {
            region_idx = g_region_table[index].region_idx;
            break;
        }
    }

    if (region_idx >= REGDOMAIN_COUNT) {
        region_idx = REGDOMAIN_COMMON;
    }
...
/*****************************************************************************
 功能描述  : 获取ini中设置的cpu_freq_level
****************************************************************************/
osal_u8 hwifi_get_cpu_freq_level(osal_void)
{
    return g_cpu_freq_level;
}

/*****************************************************************************
 功能描述      : 从ini文件读取cpu_freq_level，并设置
****************************************************************************/
osal_void hwifi_set_cpu_freq_level(osal_u8 cpu_freq_level)
{
    g_cpu_freq_level = cpu_freq_level;
}
