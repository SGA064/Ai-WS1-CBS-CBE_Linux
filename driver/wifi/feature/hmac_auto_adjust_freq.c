...
#if defined(_PRE_OS_VERSION_LINUX) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
OAL_STATIC osal_void hmac_auto_freq_limit_req_level(hmac_freq_control_stru *freq_handle)
{
    osal_u8 cfg_cpu_freq;

    cfg_cpu_freq = hwifi_get_cpu_freq_level();
    if (cfg_cpu_freq > FREQ_HIGHEST || freq_handle->req_freq_level <= cfg_cpu_freq) {
        return;
    }
    freq_handle->req_freq_level = cfg_cpu_freq;
}
#endif
...
#if defined(_PRE_OS_VERSION_LINUX) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
        hmac_auto_freq_limit_req_level(freq_handle);
#endif
        hmac_auto_freq_update_level();
    }
}
