    ...
    const mac_scan_req_stru * const scan_req_params)
{
    ...
    if ((hmac_device->curr_scan_state == MAC_SCAN_STATE_RUNNING) || (continue_flag == 0) ||
        ...
        oam_notice_log3(0, OAM_SF_SCAN, "vap[%d] hmac_scan_handle_scan_req_entry:cann't scan, vap[%d], func[0x%02x]",
            hmac_vap->vap_id, hmac_device->scan_params.vap_id, hmac_device->scan_params.scan_func);
        oam_notice_log4(0, OAM_SF_SCAN, "{hmac_scan_handle_scan_req_entry::cur[%d],req[%d],cookie[%x],flag[%d]}",
    ...
    hmac_device_stru *hmac_device)
{
    ...
    if (hal_scan_params->need_switch_back_home_channel == OSAL_TRUE) {
        ...
        hal_device_handle_event(hal_device, scan_complete_event, 0, (osal_u8 *)(hmac_vap->hal_vap));
...
OAL_STATIC osal_u32 hmac_scan_send_bcast_probe(const hmac_device_stru *hmac_device, osal_u8 band, osal_u8 index)
{
    ...
    */
    ...
    for (loop = 0; loop < send_times; loop++) {
        ...
        if (ret != OAL_SUCC) {
            oam_notice_log2(0, 0, "vap_id[%d] {hmac_scan_send_bcast_probe::hmac_mgmt_send_probe_request failed[%d].}",
                hmac_vap->vap_id, ret);
            break;
        }
    }
...
static void hmac_scan_clean_scan_record(hmac_scan_record_stru *scan_record)
{
    memset_s(&scan_record->bss_mgmt.bss_list_head, sizeof(struct osal_list_head), 0, sizeof(struct osal_list_head));
    scan_record->bss_mgmt.bss_num = 0;
    memset_s(&scan_record->bss_mgmt.bss_list_exceed_info, sizeof(hmac_bss_list_exceed_info_stru), 0,
        sizeof(hmac_bss_list_exceed_info_stru));
    memset_s(scan_record->chan_results, sizeof(scan_record->chan_results), 0, sizeof(scan_record->chan_results));
    scan_record->chan_numbers = 0;
    scan_record->device_id = 0;
    scan_record->chip_id = 0;
    scan_record->vap_id = 0;
    scan_record->scan_rsp_status = MAC_SCAN_STATUS_BUTT;
    memset_s(&scan_record->scan_start_timestamp, sizeof(oal_time_us_stru), 0, sizeof(oal_time_us_stru));
    scan_record->p_fn_cb = OSAL_NULL;
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    scan_record->is_any_scan = 0;
    scan_record->n_is_mesh_sta_scan = 0;
#endif
    scan_record->ull_cookie = 0;
    scan_record->vap_last_state = MAC_VAP_STATE_BUTT;
    scan_record->scan_start_time = 0;
    scan_record->is_chan_meas_scan = 0;
}
/*****************************************************************************
...
osal_void hmac_scan_clean_scan(hmac_scan_stru *scan)
{
    ...
    /* 2.清除扫描记录信息（链表锁不清除） */
    hmac_scan_clean_scan_record(scan_record);
    /* 3.重新初始化bss管理结果链表 */
    bss_mgmt = &(scan_record->bss_mgmt);
...
osal_u32  hmac_scan_proc_scan_req_event_etc(hmac_vap_stru *hmac_vap, osal_void *p_params)
{
    ...
    if (ret != OAL_SUCC) {
        ...
        oam_notice_log4(0, 0, "vap[%d] hmac_scan_check_is_dispatch_scan_req fail[%d] state[%d] p2p last state[%d]",
