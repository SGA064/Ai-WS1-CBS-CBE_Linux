...
#ifdef CONFIG_COMPAT
typedef struct wal_compat_android_wifi_priv_cmd {
    compat_uptr_t buf;
    int used_len;
    int total_len;
} wal_compat_android_wifi_priv_cmd;
#endif
...
#ifdef CONFIG_COMPAT
    if (is_compat_task()) {
        /* 用户空间为32-bit，内核空间为64-bit时，需要使用兼容性ioctl */
        wal_compat_android_wifi_priv_cmd compat_priv_cmd;

        if (osal_copy_from_user((osal_u8 *)&compat_priv_cmd, ifr->ifr_data, sizeof(wal_compat_android_wifi_priv_cmd))) {
            return -OAL_EINVAL;
        }
        priv_cmd.buf = compat_ptr(compat_priv_cmd.buf);
        priv_cmd.l_used_len = compat_priv_cmd.used_len;
        priv_cmd.l_total_len = compat_priv_cmd.total_len;
    } else
#endif
