...
public class AdapterProperties {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (action) {
                case NearlinkHidHost.ACTION_CONNECTION_STATE_CHANGED:
                    ...
                    break;
                case NearlinkHidHost.ACTION_PAIR_REMOVED:
                    sendPairRemoveChange(NearlinkProfile.HID_HOST, intent);
                    break;
                default:
    ...
    public void init(RemoteDevices remoteDevices) {
        ...
        filter.addAction(NearlinkHidHost.ACTION_PAIR_REMOVED);
        mService.registerReceiver(mReceiver, filter);
        mReceiverRegistered = true;
        Log.d(TAG, "init: registerReceiver");
    }

    private void sendPairRemoveChange(int profile, Intent pairRemoveIntent) {
        NearlinkDevice device = pairRemoveIntent.getParcelableExtra(NearlinkDevice.EXTRA_DEVICE);
        int status = pairRemoveIntent.getIntExtra(NearlinkHidHost.EXTRA_STATUS, -1);
        Log.d(TAG, "sendPairRemoveChange: device:" + device.getAddress()
                + " remove pair status = " + status);
        Intent intent = new Intent(NearlinkAdapter.ACTION_PAIR_REMOVED);
        intent.putExtra(NearlinkDevice.EXTRA_DEVICE, device);
        intent.putExtra(NearlinkAdapter.EXTRA_PAIR_REMOVE_STATUS, status);
