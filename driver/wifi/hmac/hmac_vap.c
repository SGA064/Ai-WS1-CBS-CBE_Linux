...

{
    ...
    OAL_NETDEVICE_SET_MAC_ADDR(net_device, mac_addr);
    ...
    hmac_device_stru *hmac_device, hmac_vap_stru *hmac_vap)
{
    ...
    for (vap_idx = 0; vap_idx < WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE; vap_idx++) {
        ...
        if ((hmac_vap_tmp->vap_mode == WLAN_VAP_MODE_BSS_STA && hmac_vap_tmp->p2p_mode == WLAN_LEGACY_VAP_MODE &&
            ...
            oam_notice_log3(0, OAM_SF_CFG, "hmac_config_add_vap_preproc:has a sta/ap now,vapid[%d] mode[%d] state[%d]",
    ...
    if (ret != OAL_SUCC) {
        oam_notice_log2(0, 0, "vap[%d]{hmac_config_add_vap_preproc:check_vap_num fail[%d]}", hmac_vap->vap_id, ret);
        return ret;
    }
...
osal_s32  hmac_config_add_vap_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (osal_unlikely(ret != OAL_SUCC)) {
        oam_notice_log2(0, OAM_SF_CFG, "vap_id[%d]{hmac_config_add_vap_etc:alloc fail[%d]}", vap_id, ret);
        return (osal_s32)ret;
    }
    ...
    oam_notice_log1(0, OAM_SF_ANY, "{hmac_config_add_vap_etc:ok!multi user idx[%d]}", hmac_vap_new->multi_user_idx);
    oam_notice_log4(0, OAM_SF_CFG, "{hmac_config_add_vap_etc::vap mode[%d] p2p mode[%d] vap id[%d] hal vap id[%d]!}",
