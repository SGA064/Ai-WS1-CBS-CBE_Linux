...
#define USB_TASK_DEINIT_WAIT_TIME_MS 2000
...
struct hcc_usb {
    ...
    struct completion rx_data_thread_comp;
...
static inline int hcc_wait_completion_timeout(struct completion *completion, int timeout)
{
    int ret = 0;
    unsigned long expire = timeout ? msecs_to_jiffies(timeout) : MAX_SCHEDULE_TIMEOUT;
#if (defined(_PRE_OS_VERSION_LITEOS) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)) || \
    (defined(_PRE_OS_VERSION_ECOS) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_ECOS == _PRE_OS_VERSION))
    ret = wait_for_completion_timeout(completion, expire);
#elif defined(_PRE_OS_VERSION_LINUX) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    ret = wait_for_completion_interruptible_timeout(completion, expire);
#else
