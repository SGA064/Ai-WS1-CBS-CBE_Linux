...
public class NearlinkSeeker {
    ...
    private final INearlinkManagerCallback sStateChangeCallback = new INearlinkManagerCallback.Stub() {
        /**
         * Called when the Nearlink service is up.
         *
         * @param nearlinkService the Nearlink service
        ...
        /**
         * Called when the Nearlink service isdown
         *
         * @throws RemoteException remote exception
         */
        public void onNearlinkServiceDown() throws RemoteException {
            synchronized (this) {
                if (DBG) {
                    Log.d(TAG, "cleanup when nearlink STATE OFF");
                }
                cleanup();
            }
        }
    ...
    public NearlinkSeeker(INearlinkManager iNearlinkManager) {
        ...
        if (this.mNearlinkAdapter != null) {
            this.mNearlinkAdapter.getNearlinkService(sStateChangeCallback);
        }
    ...
    public void startSeek(List<NearlinkSeekFilter> filters, NearlinkSeekCallback seekCallback) {
        ...
        synchronized (mScanClients) {
            if (seekCallback != null && mScanClients.containsKey(seekCallback)) {
                postStartCallbackErrorOrReturn(seekCallback, NearlinkSeekCallback.SEEK_FAILED_ALREADY_STARTED);
                return;
            }
            ...
            if (iNearlinkDiscovery == null) {
                postStartCallbackErrorOrReturn(seekCallback, NearlinkSeekCallback.SEEK_FAILED_INTERNAL_ERROR);
                return;
            }
    ...
    private int postStartCallbackErrorOrReturn(final NearlinkSeekCallback callback, final int errorCode) {
        if (callback == null) {
            return errorCode;
        } else {
            postStartCallbackError(callback, errorCode);
            return NearlinkSeekCallback.NO_ERROR;
        }
    }
    ...
    private int postStopCallbackErrorOrReturn(final NearlinkSeekCallback callback, final int errorCode) {
        if (callback == null) {
            return errorCode;
        } else {
    ...
    private class NearlinkSeekCallbackWrapper extends INearlinkSeekCallback.Stub {
        ...
        public void start() {
            synchronized (this) {
                try {
                    ...
                    postStartCallbackErrorOrReturn(nearlinkSeekCallback, NearlinkSeekCallback.SEEK_FAILED_INTERNAL_ERROR);
                }
            }
        }
        ...
        public void stop() {
            try {
                ...
                postStopCallbackErrorOrReturn(nearlinkSeekCallback, NearlinkSeekCallback.SEEK_FAILED_INTERNAL_ERROR);
            }
        }
