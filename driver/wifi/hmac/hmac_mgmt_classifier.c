...
osal_u32 hmac_send_disassoc_misc_event(hmac_vap_stru *hmac_vap, osal_u16 user_idx, osal_u16 disasoc_reason)
{
    ...
    oam_notice_log3(0, OAM_SF_ASSOC,
...
#endif
        frw_util_notifier_notify(WLAN_UTIL_NOTIFIER_EVENT_RX_BEACON, OSAL_NULL);
    }
...
{
    frw_msg msg_info = {0};
    hmac_rx_mgmt_event_stru *frame = OSAL_NULL;

    frame = (hmac_rx_mgmt_event_stru *)osal_kmalloc(rx_info->frame_len + OAL_SIZEOF(hmac_rx_mgmt_event_stru), 0);
    if (frame == OSAL_NULL) {
        oam_error_log0(0, OAM_SF_RX, "{hmac_rx_send_report_frame_msg::mgmt_data null.}");
        return;
    }
    (osal_void)memset_s(frame, rx_info->frame_len + OAL_SIZEOF(hmac_rx_mgmt_event_stru), 0,
        rx_info->frame_len + OAL_SIZEOF(hmac_rx_mgmt_event_stru));
    if (memcpy_s(frame->buf, rx_info->frame_len,
        (const osal_u8 *)mac_get_rx_cb_mac_header_addr((mac_rx_ctl_stru *)rx_info), rx_info->frame_len) != EOK) {
        oam_error_log0(0, OAM_SF_RX, "{hmac_rx_send_report_frame_msg::memcpy_s data error.}");
        osal_kfree(frame);
        return;
    }
    ...
    msg_info.data_len = OAL_SIZEOF(hmac_rx_mgmt_event_stru) + frame->len;

    (osal_void)frw_asyn_host_post_msg(msg_id, FRW_POST_PRI_LOW, hmac_vap->vap_id, &msg_info);
    osal_kfree(frame);
    ...
    hmac_vap_stru *up_vap, mac_channel_stru *channel)
{
    ...
    if (up_vap == OSAL_NULL) {
        oam_notice_log4(0, OAM_SF_SCAN, "hmac_mgmt_connect_set_channel: vap_band=%d, chan=%d chan_idx=%d, bw=%d",
    ...
    if (channel->chan_number != up_vap->channel.chan_number) {
        ...
        oam_notice_log2(0, 0, "vap_id[%d] hmac_mgmt_connect_set_channel: diff chan_num. switch off to chan %d.",
