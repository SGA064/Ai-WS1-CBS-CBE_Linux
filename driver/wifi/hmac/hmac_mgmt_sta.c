...
osal_u32 hmac_sta_wait_join_etc(hmac_vap_stru *hmac_vap, osal_void *msg)
{
    ...
    oam_notice_log4(0, OAM_SF_ASSOC,
...
osal_u32 hmac_report_ext_auth_event(hmac_vap_stru *hmac_vap)
{
    ...
    sta_ext_auth.status = MAC_SUCCESSFUL_STATUSCODE;
    {
        ...
        if (memcpy_s(sta_ext_auth.ssid, sizeof(sta_ext_auth.ssid), ssid, sta_ext_auth.ssid_len) != EOK) {
            return OAL_FAIL;
        }
    ...
    oal_netbuf_stru *auth_frame)
{
    ...
    if (auth_len == 0) {
        ...
        /* 组帧失败 */
        oam_notice_log0(0, OAM_SF_AUTH, "hmac_sta_wait_auth_proc:hmac_mgmt_encap_auth_req_etc fail.");
        ...
        if (ret != OAL_SUCC) {
            oal_netbuf_free(auth_frame);
            oam_notice_log1(0, OAM_SF_AUTH, "hmac_sta_wait_auth_proc:: hmac_tx_mgmt_send_event_etc fail[%d].", ret);
            return ret;
        }
...
OAL_STATIC osal_u32 hmac_sta_wait_auth_seq2_rx_process(hmac_vap_stru *hmac_vap, osal_u8 *mac_hdr)
{
    ...
    if (auth_rsp.status_code != MAC_SUCCESSFUL_STATUSCODE) {
        ...
        oam_notice_log2(0, OAM_SF_AUTH, "vap_id[%d] {hmac_sta_wait_auth_seq2_rx_etc::AP refuse STA auth reason[%d]!}",
...
osal_u32 hmac_sta_wait_auth_seq2_rx_etc(hmac_vap_stru *hmac_vap, osal_void *msg)
{
    ...
    if (mac_get_frame_type_and_subtype(mac_hdr) != (WLAN_FC0_SUBTYPE_AUTH | WLAN_FC0_TYPE_MGT) ||
        ...
        if ((mac_get_frame_type_and_subtype(mac_hdr) == (WLAN_FC0_SUBTYPE_DEAUTH | WLAN_FC0_TYPE_MGT)) ||
            ...
            oam_notice_log1(0, OAM_SF_RX, "{hmac_sta_wait_auth_seq2_rx_etc:recv deauth or disassoc err[%d]}", err_code);
        }
        return OAL_SUCC;
    }
    ...
    osal_u32 *asoc_frame_len)
{
    ...
    *asoc_frame_len = asoc_req_frame_len;
    ...
    if (asoc_req_frame_len == 0) {
        oam_notice_log1(0, OAM_SF_ASSOC, "vap_id[%d] {hmac_sta_wait_asoc_etc::hmac_mgmt_encap_asoc_req_sta_etc null.}",
            hmac_vap->vap_id);
        return OAL_FAIL;
    }
...
osal_u32 hmac_sta_wait_asoc_rx_etc(hmac_vap_stru *hmac_vap, osal_void *msg)
{
...
#endif
        oam_notice_log2(0, OAM_SF_ASSOC, "vap_id[%d] {hmac_sta_wait_asoc_rx_etc:: AP refuse STA assoc reason=%d.}",
            hmac_vap->vap_id, asoc_status);
    ...
    if (hmac_sta_wait_asoc_rx_fail_handle_for_sae(hmac_vap, asoc_rsp) != OAL_SUCC) {
        oam_notice_log1(0, 0, "{hmac_sta_wait_asoc_rx_fail_handle_for_sae::sae rx asoc status[%d].}", asoc_status);
        return OAL_SUCC;
    }
#endif
