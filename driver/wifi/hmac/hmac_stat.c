...
OAL_STATIC osal_u32 hmac_timeout_stat(osal_void *p_arg)
{
    ...
    hmac_device_stat_stru *hmac_device_stats = hmac_stat_get_device_stats();
    hmac_stat_throughput_stru throughput = {0};
    unref_param(p_arg);
    ...
    throughput.tx_throughput_mbps = tx_throughput_mbps;
    throughput.rx_throughput_mbps = rx_throughput_mbps;
    frw_util_notifier_notify(WLAN_UTIL_NOTIFIER_EVENT_STAT_THROUGHPUT, &throughput);
