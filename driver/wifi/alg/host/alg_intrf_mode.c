...
osal_u8 g_descend_11b_on_cnt = 0;
osal_u8 g_descend_11b_off_cnt = 0;
osal_u8 g_descend_11b_status = OSAL_FALSE;
osal_u8 g_descend_11b_config_switch = OSAL_FALSE;

/*****************************************************************************
  2 宏定义
****************************************************************************/
#define DESCEND_11B_SWITCH_ON_RSSI_TH (-50)
#define DESCEND_11B_SWITCH_OFF_RSSI_TH (-55)
#define DESCEND_11B_SWITCH_CNT_TH (10)
...
/*****************************************************************************
  3 函数实现
****************************************************************************/
OAL_STATIC osal_void alg_intrf_mode_set_11b_switch(osal_u8 descend_11b_switch)
{
    g_descend_11b_config_switch = descend_11b_switch;
}

osal_void alg_intrf_mode_rx_process(const hmac_vap_stru *hmac_vap, osal_s16 mgmt_rssi, osal_s16 data_rssi)
{
    osal_s16 rx_rssi;
    osal_u8 descend_11b_switch = g_descend_11b_status;
    ...
    if (g_descend_11b_config_switch == OSAL_FALSE) {
        return;
    }

    rx_rssi = oal_get_real_rssi(get_rssi_min(mgmt_rssi, data_rssi));
    if (rx_rssi > DESCEND_11B_SWITCH_ON_RSSI_TH) {
        g_descend_11b_on_cnt++;
        g_descend_11b_off_cnt = 0;
    } else if (rx_rssi < DESCEND_11B_SWITCH_OFF_RSSI_TH) {
        g_descend_11b_off_cnt++;
        g_descend_11b_on_cnt = 0;
    }

    if (g_descend_11b_on_cnt >= DESCEND_11B_SWITCH_CNT_TH) {
        g_descend_11b_on_cnt = 0;
        descend_11b_switch = OSAL_TRUE;
    }

    if (g_descend_11b_off_cnt >= DESCEND_11B_SWITCH_CNT_TH) {
        g_descend_11b_off_cnt = 0;
        descend_11b_switch = OSAL_FALSE;
    }

    if (g_descend_11b_status == descend_11b_switch) {
        return;
    }

    intrf_mode_info.alg_intrf_mode_cfg = MAC_ALG_INTRF_MODE_NO_11B_SWITCH;
    intrf_mode_info.value = descend_11b_switch;

    cfg_msg_init((osal_u8 *)&intrf_mode_info, OAL_SIZEOF(intrf_mode_info), OSAL_NULL, 0, &msg_info);
    ret = frw_send_msg_to_device(hmac_vap->vap_id, WLAN_MSG_H2D_CFG_ALG_INTRF_MODE_CFG, &msg_info, OSAL_FALSE);
    if (ret != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{alg_intrf_mode_rx_process::frw_send_msg_to_device failed!}");
        return;
    }

    g_descend_11b_status = descend_11b_switch;
    return;
}
...
osal_s32 alg_intrf_mode_param(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (intrf_mode->alg_intrf_mode_cfg == MAC_ALG_INTRF_MODE_NO_11B_SWITCH) {
        alg_intrf_mode_set_11b_switch((osal_u8)intrf_mode->value);
    }
    return OAL_SUCC;
}
