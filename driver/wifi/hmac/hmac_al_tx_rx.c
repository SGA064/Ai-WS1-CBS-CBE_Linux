...
OSAL_STATIC osal_void hmac_al_tx_enable(hmac_vap_stru *hmac_vap, hal_to_dmac_device_stru *hal_device,
    mac_cfg_tx_comp_stru *event_set_al_tx)
{
    osal_u8 pm_off = 0;
    ...
    hal_al_tx_cca_bypass_enable(OSAL_TRUE);
    ...
    if (hmac_alg_anti_intf_switch(hal_device, OSAL_FALSE) != OAL_SUCC) {
        oam_warning_log1(0, 0, "vap_id[%d] {hmac_al_tx_enable:anti_intf_switch fail}", hmac_vap->vap_id);
    }
...
OSAL_STATIC osal_void hmac_al_tx_disable(hmac_vap_stru *hmac_vap, hal_to_dmac_device_stru *hal_device)
{
#ifdef _PRE_WLAN_FEATURE_ANTI_INTERF
    if (hmac_alg_anti_intf_switch(hal_device, OSAL_TRUE) != OAL_SUCC) {
        oam_warning_log1(0, 0, "vap_id[%d] {hmac_al_tx_disable:anti_intf_switch fail}", hmac_vap->vap_id);
    }
    ...
    hal_al_tx_cca_bypass_enable(OSAL_FALSE);
...
osal_s32 hmac_config_set_always_tx(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (hal_device->al_tx_flag == HAL_ALWAYS_TX_RF) {
        hmac_al_tx_enable(hmac_vap, hal_device, event_set_al_tx);
    } else if (hal_device->al_tx_flag == HAL_ALWAYS_TX_DISABLE) {
        hmac_al_tx_disable(hmac_vap, hal_device);
    }
    return OAL_SUCC;
}
