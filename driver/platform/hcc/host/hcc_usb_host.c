...
{
#define USB_BULK_IN_TIMEOUT 1000
    ...
    while (rx_nums) {
        ret = usb_wlan_bulk_in(usb, buf, dev->bulk_in_max_pkt_size, USB_BULK_IN_TIMEOUT);
        if (ret < 0) {
            break;
        }
        rx_nums--;
...
static td_s32 usb_xfer_wifi_rx_data_thread(td_void *data)
{
    ...
    osal_atomic_set(&g_usb->usb_rx_cond, 1);
    init_completion(&g_usb->rx_data_thread_comp);
    ...
    complete(&g_usb->rx_data_thread_comp);
    return ret;
}
...
static td_void usb_task_rx_data_deinit(td_void)
{
    ...
    ret = hcc_wait_completion_timeout(&g_usb->rx_data_thread_comp, USB_TASK_DEINIT_WAIT_TIME_MS);
    oal_usb_log(BUS_LOG_INFO, "hcc_wait_completion_timeout ret:%d\r\n", ret);
    osal_kthread_destroy(g_usb->rx_data_handle, 0);

    g_usb->is_wlan_rx_thread_running = TSK_USB_EXIT;
    g_usb->rx_data_handle = TD_NULL;
#if defined(_PRE_OS_VERSION_ECOS) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_ECOS == _PRE_OS_VERSION)
    destroy_complete(&g_usb->rx_data_thread_comp);
#endif
}
...
static td_void oal_usb_disconnect(struct usb_interface *interface)
{
    ...
    if (old_state == BUS_USB_SUSPEND) {
        ...
        osal_atomic_set(&g_usb->usb_suspend, 0);
    }
