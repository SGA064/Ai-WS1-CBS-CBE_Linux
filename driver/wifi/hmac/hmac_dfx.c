...
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT OAL_STATIC osal_bool dfx_user_rate_rx_info_record(osal_void *notify_data)
{
    ...
    cb_ctrl = (dmac_rx_ctl_stru *)notify_data;
    id = cb_ctrl->rx_info.ta_user_idx;
...
osal_u32 hmac_dfx_init(osal_void)
{
    ...
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_USER_RATE_RX_INFO, dfx_user_rate_rx_info_record);
    /* DFX:用户连接统计，默认使能 */
    dfx_user_rd_init();
    /* 消息注册 */
#if defined(_PRE_OS_VERSION_LITEOS) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
...
osal_void hmac_dfx_deinit(osal_void)
{
    ...
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_USER_RATE_RX_INFO, dfx_user_rate_rx_info_record);
    /* DFX:用户连接统计，去使能 */
    dfx_user_rd_exit();
