    ...
    struct usb_urb_context *ctx)
{
    ...
    wait_ret = hcc_wait_completion_timeout(&ctx->done, timeout);
    if (wait_ret == 0) {
        usb_kill_urb(urb);
...
#ifdef CONFIG_HCC_CPU_OPTIMIZE
    if (g_urb_tx_work_queue.qlen == URB_TX_MAX_NUM) {
        struct hcc_usb *usb_tmp = usb_get_handle();
        hcc_sched_transfer((hcc_handler *)usb_tmp->pst_bus->hcc);
        return;
    }
#endif
