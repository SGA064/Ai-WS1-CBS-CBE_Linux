...
OAL_STATIC bsle_cfg_cmd g_ast_bsle_config[] = {
    ...
    {"bsle_coex_param",                BSLE_CFG_INI_BSLE_COEX_PARAM},
    {OAL_PTR_NULL, 0}
};
...
td_u32 hcc_adapt_bsle_msg_data_col_cmd_tx_proc(td_void *cmd, td_u32 len)
{
    ...
    tx_buf = (oal_uint8 *)osal_kzalloc(BSLE_LARGE_NETBUF_SIZE, OSAL_GFP_KERNEL);
    if (tx_buf == NULL) {
    ...
    oal_int32 val)
{
    switch (bsle_cfg_id) {
        ...
        case BSLE_CFG_INI_BSLE_USE_FLASH:
            ...
            break;
        case BSLE_CFG_INI_BSLE_COEX_PARAM:
            bsle_custom_data->bsle_coex_param = (uint32_t)val;
            break;
        default:
...
#endif
    td_u8 *tx_buf;
    tx_buf = osal_kzalloc(buf_len, OSAL_GFP_KERNEL);
    if (tx_buf == NULL) {
