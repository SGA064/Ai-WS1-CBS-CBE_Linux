...
hmac_temp_pri_custom_stru g_dmac_temp_protect_pri_custom = {
    ...
    /* 过温保护过程中是否需要减少调度 */
    HMAC_TEMP_PROTECT_REDUCE_SCHEDULE_ENABLE,
    /* 过温保护过程中是否需要限制MCS及重传次数 */
    HMAC_TEMP_PROTECT_LIMIT_MCS_RETRY_ENABLE,
    0, /* reserve */
    /* 过温保护的恢复安全水线 */
...
osal_void alg_temp_stat_record(alg_temp_protect_state cur_state, osal_u32 cur_duty_cyc)
{
    ...
    temp_protect->stat_record[idx].limit_mcs_retry = temp_protect->limit_mcs_retry;
...
OSAL_STATIC osal_void alg_temp_protect_softap_act(alg_temp_protect_stru *temp_protect, osal_u8 need_limit)
{
    ...
    hal_to_dmac_device_stru *hal_device = hal_chip_get_hal_device();
    hmac_temp_pri_custom_stru *temp_pri_custom = hmac_get_pri_custom();
    osal_u8 is_softap_exist = OSAL_FALSE;

    if (temp_pri_custom->limit_mcs_retry_enable == OSAL_FALSE) {
        return;
    }

    if (temp_protect->limit_mcs_retry == need_limit) {
        return;
    }
    ...
    for (vap_index = 0; vap_index < vap_num; vap_index++) {
        ...
        if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_AP) {
            is_softap_exist = OSAL_TRUE;
        }
    }

    if (is_softap_exist == OSAL_FALSE) {
        return;
    }

    /* 限制MCS及重传次数 */
    temp_protect->limit_mcs_retry = need_limit;
    alg_temp_protect_send_msg(need_limit, WLAN_MSG_H2D_C_CFG_DEVICE_TEMP_LIMIT_MCS_RETRY);
    oam_warning_log1(0, OAM_SF_ANY, "{alg_temp_protect_over_sta_act::limit_mcs_retry[%d]}", need_limit);
}
...
osal_void alg_temp_protect_safe_entry(osal_void *ctx)
{
    ...
    /* softAP模式下恢复最低MCS及重传次数限制 */
    alg_temp_protect_softap_act(temp_protect, OSAL_FALSE);
...
osal_void alg_temp_protect_safe_sta_act(alg_temp_protect_stru *temp_protect, osal_u32 cur_duty_cyc)
{
    ...
    if (cur_duty_cyc > temp_protect->pre_set_duty_cyc) {
        /* 区分开写是为防止出现小于0的情况 聚合度是无符号数 */
        if (temp_protect->temp_pro_aggr_size >=
            temp_protect->temp_pro_min_aggr_size + ALG_TEMP_PROTECT_AGGR_SIZE_STEP) {
            temp_protect->temp_pro_aggr_size -= ALG_TEMP_PROTECT_AGGR_SIZE_STEP;
        } else {
            temp_protect->temp_pro_aggr_size = temp_protect->temp_pro_min_aggr_size;
        }
        /* 聚合度改变后 发消息给device侧 */
...
osal_void alg_temp_protect_over_entry(osal_void *ctx)
{
    ...
    /* softAP模式下限制最低MCS及重传次数以防止功耗过大 */
    alg_temp_protect_softap_act(temp_protect, OSAL_TRUE);
    return;
}
...
osal_void alg_temp_protect_over_sta_act(alg_temp_protect_stru *temp_protect, osal_u32 cur_duty_cyc)
{
    ...
    if (temp_protect->temp_pro_aggr_size >= (temp_protect->temp_pro_min_aggr_size + ALG_TEMP_PROTECT_AGGR_SIZE_STEP)) {
    ...
    if (temp_protect->temp_pro_aggr_size > temp_protect->temp_pro_min_aggr_size) {
        temp_protect->temp_pro_aggr_size = temp_protect->temp_pro_min_aggr_size;
        alg_temp_protect_send_msg(temp_protect->temp_pro_aggr_size, WLAN_MSG_H2D_C_CFG_DEVICE_TEMP_AGGR_CHANGE);
        return;
    }
    ...
    if ((temp_protect->schedule_reduce == OSAL_FALSE) && (temp_pri_custom->temp_pro_reduce_sch_enable == OSAL_TRUE)) {
    ...
    /* 等待一次10次采样后，恢复为3次 */
    if (temp_pri_custom->temp_pro_reduce_sch_enable == OSAL_TRUE) {
        temp_protect->times_th = ALG_TEMP_PROTECT_TIMES_TH_OVER;
        alg_temp_protect_data_clear();
    }
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
OSAL_STATIC osal_void alg_temp_protect_level_adjustment(alg_temp_protect_stru *temp_protect)
{
    osal_s32 temp_pro_level = ALG_TEMP_PROTECT_LEVEL_BUTT;
    hmac_temp_pri_custom_stru *temp_pri_custom = hmac_get_pri_custom();

    if (hwifi_get_init_priv_value(WLAN_CFG_PRIV_TEMP_PROTECT_LEVEL, &temp_pro_level) != OAL_SUCC) {
        return;
    }

    if (temp_pro_level == ALG_TEMP_PROTECT_LEVEL0) {
        temp_protect->temp_pro_min_aggr_size = ALG_TEMP_PROTECT_MIN_AGGR_SIZE << 1;
        temp_pri_custom->temp_pro_reduce_sch_enable = OSAL_FALSE;
        hal_power_set_pow_tpc_level_table(HAL_POW_MIN_POW_LEVEL, WLAN_BAND_2G, ALG_TEMP_PROTECT_PWR_REDUCE_LEVEL0);
    } else if (temp_pro_level == ALG_TEMP_PROTECT_LEVEL2) {
        temp_pri_custom->limit_mcs_retry_enable = OSAL_TRUE;
    }
}
#endif
...
osal_void alg_temp_protect_init(osal_void)
{
    ...
    temp_protect->temp_pro_aggr_size = ALG_TEMP_PROTECT_MAX_AGGR_SIZE;
    temp_protect->temp_pro_min_aggr_size = ALG_TEMP_PROTECT_MIN_AGGR_SIZE;
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    /* 根据客户配置的温度保护档位，进行策略调整 */
    alg_temp_protect_level_adjustment(temp_protect);
#endif
...
osal_u32 alg_temp_protect_timer_hander(osal_void *arg)
{
    ...
    if (temp_protect->times < (temp_protect->times_th)) {
        ...
        if (temp_protect->ave_temp >= ALG_TEMP_PROTECT_OVER_TH) {
            oam_warning_log1(0, OAM_SF_ANY, "{alg_temp_protect_timer_hander::temperature over %d}",
...
osal_void alg_temp_protect_info_get(osal_void)
{
    ...
    for (idx = 0; idx < ALG_TEMP_PROTECT_RECORD_AMOUNT; idx++) {
        ...
        wifi_printf("sche_reduce=%u, limit_mcs_retry=%u",
            temp_protect->stat_record[idx].schedule_reduce, temp_protect->stat_record[idx].limit_mcs_retry);
    ...
    for (idx = 0; idx < ALG_TEMP_PROTECT_RECORD_AMOUNT; idx++) {
        ...
        oam_warning_log4(0, OAM_SF_ANY, "{ave_temp=%d,cur_state=%u,duty_set=%u,duty_cur=%u,}",
            ...
            temp_protect->stat_record[idx].pre_set_duty_cyc,
            temp_protect->stat_record[idx].cur_duty_cyc);
        oam_warning_log2(0, OAM_SF_ANY, "{sche_reduce=%u, limit_mcs_retry=%u}",
            temp_protect->stat_record[idx].schedule_reduce, temp_protect->stat_record[idx].limit_mcs_retry);
