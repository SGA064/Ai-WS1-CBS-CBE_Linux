    ...
    oal_cfg80211_scan_request_stru *request)
{
    ...
    if ((hmac_vap == NULL) || (((hmac_vap->vap_state == MAC_VAP_STATE_UP) ||
        ...
        oam_notice_log1(0, OAM_SF_CFG, "{wal_cfg80211_scan_start::do not scan, null %d}", (hmac_vap == NULL) ? 1 : 0);
        return -OAL_EFAIL;
    }
    ...
    if (osal_wait_interruptible(&scan_mgmt->wait_queue, wal_cfg80211_scan_start_wait_condition,
        scan_mgmt) < 0) {
        oam_notice_log1(0, OAM_SF_CFG, "vap_id[%d] {uapi_cfg80211_scan::start new scan fail}", hmac_vap->vap_id);
        return -OAL_EFAIL;
    }
    ...
    oal_net_device_stru *net_device, oal_cfg80211_conn_stru *sme)
{
...
#ifdef _PRE_WLAN_FEATURE_DFR
    if (plat_is_device_in_recovery() == OSAL_TRUE) {
        oam_notice_log1(0, OAM_SF_ANY, "uapi_cfg80211_connect::dfr ing... status[%d]", wal_dfr_get_status());
        return -OAL_EFAIL;
    }
#endif
    /* begin:禁止采用p2p device设备启动关联 */
    if (wal_is_p2p_device(net_device)) {
        oam_notice_log0(0, OAM_SF_ANY, "uapi_cfg80211_connect:connect stop, p2p device should not connect.");
        return -OAL_EINVAL;
    }
    ...
    if (ret != OAL_SUCC) {
        oam_notice_log1(0, OAM_SF_ANY, "{uapi_cfg80211_connect::wal_cfg80211_start_connect_etc fail %d!}", ret);
        return -OAL_EFAIL;
    }
...
#endif
{
    ...
    if (type == NL80211_IFTYPE_AP) {
        ...
        return (wal_setup_ap_etc(net_dev) == 0) ? OAL_SUCC : -OAL_EFAIL;
    }
#endif
#endif
