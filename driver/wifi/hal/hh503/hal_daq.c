...
osal_void hal_set_diag_ctl_open(osal_void)
{
    u_sysldo_crg_cken_ctl crg_cken;
    u_sysldo_soft_rst_ctl soft_rst;

    // 0x1 -> 0x4001F100 bit 26/13/11/5
    crg_cken.u32 = hal_reg_read(CLK_RST_CRG_CFG_SYSLDO_CRG_CKEN_CTL_REG);
    crg_cken.bits.diag_ctl_cken = 1;
    crg_cken.bits.monitor_wr_cken = 1;
    crg_cken.bits.ref_1m_cken = 1;
    crg_cken.bits.cali_ref_cken = 1;
    hal_reg_write(CLK_RST_CRG_CFG_SYSLDO_CRG_CKEN_CTL_REG, crg_cken.u32);

    // 0x7 -> 0x4001F120 bit 21:19 (3 bits)
    soft_rst.u32 = hal_reg_read(CLK_RST_CRG_CFG_SYSLDO_SOFT_RST_CTL_REG);
    soft_rst.bits.soft_rst_cpu_trace_n = 1;
    soft_rst.bits.xCiNLw_CLwSihBLi_wh_ = 1;
    soft_rst.bits.soft_rst_diag_n = 1;
    hal_reg_write(CLK_RST_CRG_CFG_SYSLDO_SOFT_RST_CTL_REG, soft_rst.u32);
}
    ...
    hal_to_dmac_device_mac_test_stru *hal_mac_test_device)
{
    ...
    /* step0 使能diag ctl clock */
    hal_set_diag_ctl_open();
