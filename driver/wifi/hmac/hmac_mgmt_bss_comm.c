    ...
    osal_slong l_freq)
{
    ...
    mgmt_frame = (hmac_rx_mgmt_event_stru *)oal_memalloc(len + OAL_SIZEOF(hmac_rx_mgmt_event_stru));
    if (mgmt_frame == OAL_PTR_NULL) {
        oam_error_log1(0, OAM_SF_SCAN, "vap_id[%d] {hmac_send_mgmt_to_host_etc::mgmt_frame null.}", hmac_vap->vap_id);
        /* 管理帧丢弃 */
        hmac_dft_print_drop_frame_info(THIS_FILE_ID, __LINE__, 1, buf);
        return;
    }
    memset_s(mgmt_frame, len + OAL_SIZEOF(hmac_rx_mgmt_event_stru), 0, len + OAL_SIZEOF(hmac_rx_mgmt_event_stru));
    (osal_void)memcpy_s(mgmt_frame->buf, len, (osal_u8 *)mac_get_rx_cb_mac_header_addr(rx_info), len);
    ...
    msg_info.data_len = len + OAL_SIZEOF(hmac_rx_mgmt_event_stru);
