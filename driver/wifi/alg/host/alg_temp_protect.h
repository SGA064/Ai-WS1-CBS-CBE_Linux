...
#define ALG_TEMP_PROTECT_PWR_REDUCE_LEVEL0 (-9)   /* 保护档位0档时的功率降低幅度 */
...
#define HMAC_TEMP_PROTECT_REDUCE_SCHEDULE_ENABLE (OSAL_TRUE)
#define HMAC_TEMP_PROTECT_LIMIT_MCS_RETRY_ENABLE (OSAL_FALSE)
...
/* 档位枚举 */
typedef enum {
    ALG_TEMP_PROTECT_LEVEL0  = 0,
    ALG_TEMP_PROTECT_DEFAULT = 1,
    ALG_TEMP_PROTECT_LEVEL2  = 2,

    ALG_TEMP_PROTECT_LEVEL_BUTT
} alg_temp_protect_level_enum;
typedef osal_u16 alg_temp_protect_level_enum_uint16;
...
typedef struct {
    ...
    osal_u8   temp_pro_reduce_sch_enable : 1; /* 过温保护中是否需要减少调度 */
    osal_u8   limit_mcs_retry_enable : 1; /* 过温保护过程中是否需要限制MCS及重传次数 */
    osal_u8   rsv : 4;
...
typedef struct {
    ...
    osal_u8 limit_mcs_retry;                    /* 减小功率的使能开关 */
...
typedef struct {
    ...
    osal_u8 temp_pro_min_aggr_size;
