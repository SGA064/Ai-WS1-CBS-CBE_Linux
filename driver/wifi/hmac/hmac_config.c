...
OAL_STATIC osal_void hmac_config_get_ap_user_info_printf(hmac_vap_stru *hmac_vap, osal_s8 *sta_list_buff, osal_u32 len)
{
    ...
    osal_list_for_each(head, &(hmac_vap->mac_user_list_head)) {
        ...
        oam_info_log4(0, OAM_SF_CFG, "hmac_config_get_sta_list_etc,STA:%02X:%02X:%02X:%02X:XX:XX",
            hmac_user->user_mac_addr[0], hmac_user->user_mac_addr[1], /* 打印第 0, 1 位mac */
            hmac_user->user_mac_addr[2], hmac_user->user_mac_addr[3]); /* 打印第 2, 3 位mac */
...
OAL_STATIC osal_u32 hmac_config_connect_check_bss_dscr_channel(mac_bss_dscr_stru *bss_dscr, hmac_vap_stru *hmac_vap)
{
    ...
    if ((hmac_device_check_5g_enable(hmac_vap->device_id) == OAL_FALSE) && (frame_channel > MAX_CHANNEL_NUM_FREQ_2G)) {
        oam_notice_log2(0, OAM_SF_CFG, "{hmac_config_connect_check_bss_dscr_channel not support 5G, vap[%d] chan[%d]}",
    ...
    if (hmac_is_channel_num_valid_etc(mac_get_band_by_channel_num(frame_channel), frame_channel) != OAL_SUCC) {
        oam_notice_log2(0, OAM_SF_CFG, "{hmac_config_connect_check_bss_dscr_channel vap[%d] channel[%d] invalid.}",
            hmac_vap->vap_id, frame_channel);
    ...
    hmac_vap_stru *hmac_vap, mac_conn_param_stru *connect_param)
{
    ...
    if (osal_memcmp(connect_param->ssid, bss_dscr->ac_ssid, (osal_s32)connect_param->ssid_len) != 0) {
        oam_notice_log1(0, OAM_SF_CFG, "{vap[%d] hmac_config_connect_check_bss_dscr ssid not same}", hmac_vap->vap_id);
        return OAL_FAIL;
    }
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
#if defined(_PRE_OS_VERSION_LINUX) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
OAL_STATIC osal_void hmac_cpu_freq_level_init(osal_void)
{
    osal_u32 ret;
    osal_u8 cfg_cpu_freq;

    cfg_cpu_freq = hwifi_get_cpu_freq_level();
    if (cfg_cpu_freq > FREQ_HIGHEST) {
        return;
    }

    ret = pm_crg_svc_vote(0, cfg_cpu_freq); // 0: CRG sid of WiFi
    oam_warning_log1(0, OAM_SF_ANY, "{hmac_cpu_freq_level_init: ret[%u]}", ret);
}
#endif
...
#endif
#ifdef _PRE_WLAN_INFO_REPORT
        {WLAN_MSG_W2H_C_CFG_WLAN_INFO_REPORT, hmac_wlan_info_report_proc},
#endif
    };
...
osal_s32 hmac_config_connect(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    oam_notice_log3(0, OAM_SF_CFG, "vap_id[%d] hmac_config_connect::c_ant0_rssi[%d],c_ant1_rssi[%d]", hmac_vap->vap_id,
