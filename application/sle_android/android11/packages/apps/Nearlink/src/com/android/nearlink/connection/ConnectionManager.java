...
public class ConnectionManager {
    ...
    private void updateConnDeviceInfo(ConnectionDevice device) {
        synchronized (obj) {
            ...
            int preConnState = preConnDevice == null
                ? NearlinkConstant.SLE_ACB_STATE_NONE : preConnDevice.getConnState();
            int prePairState = preConnDevice == null
                ? NearlinkConstant.SLE_PAIR_NONE : preConnDevice.getPairState();
            int newConnState = device.getConnState();
            int newPairState = device.getPairState();

            if (newPairState != prePairState) {
                connMap.put(device.getKey(), device);
            }

            if (newConnState != preConnState) {
                if (newConnState == NearlinkConstant.SLE_ACB_STATE_CONNECTED) {
                    device.setHasConnected(HAS_CONNECTED);
                } else if (preConnDevice != null) {
                    device.setHasConnected(preConnDevice.getHasConnected());
                }
                connMap.put(device.getKey(), device);

                // 本地记录连接信息(只在连接和正常断连时保存)
                boolean isNeedToPersistent = newConnState == NearlinkConstant.SLE_ACB_STATE_CONNECTED
                    || (newConnState == NearlinkConstant.SLE_ACB_STATE_DISCONNECTED
                        && preConnState != NearlinkConstant.SLE_ACB_STATE_CONNECTING);
                if (isNeedToPersistent) {
                    writeToDisk(connMap);
                }
            }
            if (newConnState == NearlinkConstant.SLE_ACB_STATE_CONNECTED
