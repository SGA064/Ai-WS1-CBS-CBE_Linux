...
OSAL_STATIC osal_bool hmac_btcoex_hal_wifi_busy_notify(osal_void *notify_data)
{
    osal_u8 wlan_busy;
    ...
    if (g_btcoex_stat_flag == OSAL_FALSE) {
        g_btcoex_throughput.tx_throughput = (osal_u16)(throughput->tx_throughput_mbps);
        g_btcoex_throughput.rx_throughput = (osal_u16)(throughput->rx_throughput_mbps);
        g_btcoex_stat_flag = OSAL_TRUE;
        return OAL_TRUE;
    }

    /* 2,计算tx吞吐量平均值 */
    throughput->tx_throughput_mbps = (throughput->tx_throughput_mbps + g_btcoex_throughput.tx_throughput) / 2;
    /* 2,计算rx吞吐量平均值 */
    throughput->rx_throughput_mbps = (throughput->rx_throughput_mbps + g_btcoex_throughput.rx_throughput) / 2;
    throughput_mbps = throughput->tx_throughput_mbps + throughput->rx_throughput_mbps;
...
OSAL_STATIC osal_u32 hmac_btcoex_init_ect(osal_void)
{
    ...
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_STAT_THROUGHPUT, hmac_btcoex_hal_wifi_busy_notify);
    /* 对外接口注册 */
...
OSAL_STATIC osal_void hmac_btcoex_deinit_ect(osal_void)
{
    ...
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_STAT_THROUGHPUT, hmac_btcoex_hal_wifi_busy_notify);
    /* 对外接口去注册 */
