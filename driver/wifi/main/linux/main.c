...
static void hwifi_cfg_cpu_freq_level_init(void)
{
    osal_s32 priv_value = 0;
    osal_s32 ret;

    ret = hwifi_get_init_priv_value(WLAN_CFG_PRIV_CPU_FREQ_LEVEL, &priv_value);
    if (ret == OAL_SUCC) {
        hwifi_set_cpu_freq_level((osal_u8)priv_value);
    }
    return;
}
...
osal_s32 host_module_init_etc(void)
{
    ...
    if (ret != OAL_SUCC && ret != OAL_ERR_CODE_ALREADY_OPEN) {
        builder_module_exit(BIT6 | BIT7 | BIT8);
        return ret;
    }
