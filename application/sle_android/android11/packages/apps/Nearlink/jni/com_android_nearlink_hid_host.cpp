...
static jmethodID method_onPairRemoved;
...
{
    ALOGE("connection_state_callback address = 0x%02X:0x%02X:**:**:0x%02X:0x%02X, addr_type = 0x%02X",
        address->addr[ARRAY_0], address->addr[ARRAY_1], address->addr[ARRAY_4], address->addr[ARRAY_5],
        address->type);
    ...
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onConnectStateChanged,
        addr.get(), (jint)(address->type), (jint)conn_state, (jint)pair_state, (jint)disc_reason);
}

static void pair_state_removed_callback(const sle_addr_t *address, errcode_t status)
{
    ALOGD("%s 0x%02X:0x%02X:**:**:0x%02X:0x%02X, status = 0x%02X", __func__,
        address->addr[ARRAY_0], address->addr[ARRAY_1], address->addr[ARRAY_4], address->addr[ARRAY_5], status);
    CallbackEnv sCallbackEnv(__func__);
    if (!sCallbackEnv.valid()) {
        return;
    }
    ...
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onPairRemoved, addr.get(), (jint)(address->type), (jint)status);
}
...
static void handshake_callback(uint16_t conn_id, const sle_addr_t *address, sle_acb_state_t conn_state)
{
    ...
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onHandshake, addr.get(), (jint)(address->type), (jint)conn_state);
}

static sle_hh_callbacks_t sNearlinkHidCallbacks = {connection_state_callback,
    pair_state_removed_callback, handshake_callback};
...
static void classInitNative(JNIEnv *env, jclass clazz)
{
    ...
    method_onConnectStateChanged = env->GetMethodID(clazz, "onConnectStateChanged", "([BIIII)V");
    method_onPairRemoved = env->GetMethodID(clazz, "onPairRemoved", "([BII)V");
    method_onHandshake = env->GetMethodID(clazz, "onHandshake", "([BII)V");
...
static jboolean connectHidNative(JNIEnv *env, jobject object, jbyteArray address, jint type)
{
    ALOGD("connectHidNative");
    sle_addr_t sAddr;
    env->GetByteArrayRegion(address, 0, SLE_ADDR_LEN, (jbyte *)sAddr.addr);
    sAddr.type = type;
    ALOGD("connect address = 0x%02X:0x%02X:**:**:0x%02X:0x%02X, type:0x%02X",
          sAddr.addr[ARRAY_0], sAddr.addr[ARRAY_1], sAddr.addr[ARRAY_4], sAddr.addr[ARRAY_5], type);
    ...
    if (result == ERRCODE_SLE_SUCCESS && out.state == AAPI_OUT_STATUS_FINISHED) {
        ...
        env->CallVoidMethod(mCallbacksObj,method_onConnectStateChanged,
            address, type, out.conn_state, out.pair_state, 0x0);
        return JNI_TRUE;
    }
    if (result != ERRCODE_SLE_SUCCESS) {
...
static jboolean disconnectHidNative(JNIEnv *env, jobject object, jbyteArray address, jint type)
{
    ALOGD("disconnectHidNative");
    sle_addr_t sAddr;
    env->GetByteArrayRegion(address, 0, SLE_ADDR_LEN, (jbyte *)sAddr.addr);
    sAddr.type = type;
    ALOGD("disconnect address = 0x%02X:0x%02X:**:**:0x%02X:0x%02X, type:0x%02X",
          sAddr.addr[ARRAY_0], sAddr.addr[ARRAY_1], sAddr.addr[ARRAY_4], sAddr.addr[ARRAY_5], type);
    uint8_t out;
...
static JNINativeMethod sMethods[] = {
    ...
    {"connectHidNative", "([BI)Z", (void *)connectHidNative},
    {"disconnectHidNative", "([BI)Z", (void *)disconnectHidNative},
