    ...
    oal_bool_enum_uint8 is_protected)
{
    ...
    /* 增加接收到去认证帧时的维测信息 */
    oam_notice_log1(0, OAM_SF_CONN, "{hmac_ap_rx_deauth_req:: DEAUTH rx, reason code = %d}", err_code);
    oam_notice_log4(0, OAM_SF_CONN, "{hmac_ap_rx_deauth_req:: DEAUTH rx, sa[%2X:%2X:%2X:%2X:XX:XX]}",
    ...
    osal_u32 mac_hdr_len, osal_u8 *payload, osal_u32 payload_len, oal_bool_enum_uint8 is_protected)
{
    ...
    oam_notice_log1(0, OAM_SF_CONN, "{DISASSOC rx, err_code[%d].}", *((osal_u16 *)(mac_hdr + MAC_80211_FRAME_LEN)));
    /* 0、1、2、3表示MAC相应位数 */
    oam_notice_log4(0, OAM_SF_CONN, "{recv disassoc frame %02X:%02X:%02X:%02X:XX:XX.}", sa[0], sa[1], sa[2], sa[3]);
